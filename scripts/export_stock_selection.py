"""IDE entry point: identify stocks to trade and export the result workbook."""
...
def main() -> None:
    ...
    result, output = export_stock_selection_report(
    ...
    print(f"Stock-selection workbook exported: {output}")
    print(
        f"Stocks: {result.stock_count}; "
        f"K: {result.clustering_result.requested_cluster_count}"
    )
    print(
        f"Winners: {result.quality.winner_count}; "
