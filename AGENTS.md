...
6. 单个决策日的只做多组合权重；
7. 固定份额跨日持仓、`l=3` 定期再平衡、组合复利 `q=5%` 提前再平衡、SPY 基准及无成本绩效评价。
...
- 回测起止日均为显式指定、包含在结果中的 SPY 收益日期；
- 初始 `as_of_date=T` 权重只使用至 `T-1` 的数据，并获得日期为 `T` 的 close-to-close 收益；
- 两次事件之间固定经济份额，权重自然漂移，不做隐含每日再平衡；
- 每轮获得 3 个日收益后定期换仓；若第 1 或第 2 日组合本轮复利净值收益达到 `5%`，提前换仓；
- 第 3 日同时达到阈值时按定期换仓记录；
...
加入交易成本、滑点、FF12 或其他风险控制前，应分别确认成本计提对象、成交价、退市收益、现金利率、行业标签来源和对照实验。不得把这些扩展静默混入当前无成本基线。
