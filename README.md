...
确认重建已有数据库时显式增加 `--replace`。程序先在同目录构建临时 DuckDB，只有完整成功且动态股票池非空后才原子替换正式文件；股票下载全部失败、股票池为空或其他构建失败时，运行标记为失败并保留旧数据库。
...
Excel 默认采用精简视图：`Summary` 把策略与 SPY 指标并排展示；`Period_Performance` 按自然年和自然月复利汇总策略、SPY、超额收益、年化波动率、Sharpe 和区间最大回撤，并用以零为中心的红—白—绿色阶展示策略月收益及相对 SPY 的月度超额收益热力图；日度、再平衡、组合动作和 lot 表先显示常用字段，现金拆分、冻结价值、FIFO 中间字段等技术列保留在各表右侧并默认隐藏，需要深度审计时可在 Excel 中取消隐藏。`Missing_Data_Audit` 没有异常记录时整张表默认隐藏，有记录时自动显示。期间汇总只从同一组日收益派生，该调整不改变回测计算、持仓或时间语义。
...
回测和 grid 的内部 `BacktestTarget` 只携带正权重股票；winner、neutral 和其他零权重行仍完整保留在单决策日 portfolio-weight 报告中，因此该优化不改变目标权重、持仓、收益或审计口径。

策略与 SPY 使用相同日期，风险利率和现金收益均为零。报告给出复利年化收益、年化 Sharpe 和按负收益样本标准差计算的 Sortino。回测结果只在内存中计算并导出 Excel，不写入 DuckDB。

所有 Excel 报告先保存到输出目录中的临时文件，只有保存完整成功后才原子替换目标文件；保存失败时删除临时文件，不创建新输出，也不修改已有文件。
...
- Excel 年度/月度表现、月收益与超额收益热力图、精简视图、隐藏技术审计列、合并动作表、持仓周期表、禁止静默覆盖和保存失败时保留已有文件。
